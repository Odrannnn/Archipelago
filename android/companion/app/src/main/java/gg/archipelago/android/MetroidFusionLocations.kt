 ...
 * Empty entries are reserved/unused bits and must retain their positions.
 */
object MetroidFusionLocations {
    val minor = """
    ...
    fun checkedNames(snapshot: MetroidFusionProfile.Snapshot): Set<String> = buildSet {
        addChecked(minor, snapshot.minorLocationBits)
        addChecked(major, snapshot.majorLocationBits)
    }

    private fun MutableSet<String>.addChecked(names: List<String>, bits: ByteArray) {
        names.forEachIndexed { index, name ->
            if (name != "<unused>" && index / 8 < bits.size &&
